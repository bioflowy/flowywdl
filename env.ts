...
export class Binding<T> {
  ...
  export class Bindings<T> implements Iterable<Binding<T>> {
    ...
    constructor(binding: Binding<T> | null = null, next: Bindings<T> | null = null) {
      if (!binding && next) {
        throw new Error("Invalid Bindings construction: binding must exist if next exists");
      }
    ...
    *[Symbol.iterator](): Iterator<Binding<T>> {
    ...
    get namespaces(): Set<string> {
      if (this._namespaces === null) {
        this._namespaces = this._next ? new Set(this._next.namespaces) : new Set();
